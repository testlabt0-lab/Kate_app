                      ...
                      onSelected: (val) {
                        setState(() {
                          if (val == 'delete') items.removeAt(index);
                            ...
                            // Update status trick (create new instance since properties are final)
                ...
                children: [
                  TypeAheadField<Farmer>(
                    ...
                    emptyBuilder: (context) => ListTile(
                      leading: const Icon(Icons.add_circle, color: Colors.blue), title: Text('إضافة مزارع جديد: "${farmerController.text}"'),
                      onTap: () async { await provider.addFarmer(farmerController.text, null); setDialogState(() => selectedFarmer = provider.farmers.last); },
                    ),
                  ),
                  ...
                  TypeAheadField<Agent>(
                    ...
                    emptyBuilder: (context) => ListTile(
                      leading: const Icon(Icons.add_circle, color: Colors.blue), title: Text('إضافة وكيل جديد: "${agentController.text}"'),
                      onTap: () async { await provider.addAgent(agentController.text, null); setDialogState(() => selectedAgent = provider.agents.last); },
                    ),
                  ),
                  const SizedBox(height: 10),
  ...
  void _showDuplicateFarmerWarning(BuildContext context, Farmer farmer, Agent agent, List<Map<String, dynamic>> qatData, int boxes) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('المزارع مسجل بالفعل. ماذا تريد؟'),
        actions: [
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(onPressed: () { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('تم نسخ الكشف!'))); Navigator.pop(context); }, child: const Text('مشاركة / نسخ')),
        ],
      ),
    );
  }
